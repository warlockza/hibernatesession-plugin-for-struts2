	...
	public String intercept(ActionInvocation invocation) throws Exception {
		...
		try {
			...
			for (Field field:fields) {
				...
				if (Pattern.matches(HibernateSessionPluginUtils.wildcardToRegex(targets[0]), field.getName())) {
					targets[0] = field.getName();
					String trueTarget = Arrays.toString(targets).replace("[", "").replace("]", "").replace(", ",".");
					invocation.getStack().setValue(trueTarget, hibernateSesssion);
					targetFound = true;
				}
			}
			if (!targetFound) {
				throw new Exception("Target \""+target+"\" not found in Value Stack ("+invocation.getAction().getClass().getName()+") for Hibernate Session.");
			}
			...
			System.err.println("Error! Could not open Hibernate Session: "+e.getMessage());
			e.printStackTrace();
			throw e;
		}
